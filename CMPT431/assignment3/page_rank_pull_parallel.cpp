...
void page_rank_pull_parallel(Graph *g, uint iterStart, uint iterEnd, uint thread_id, uint n, double *thread_time, PageRankType *pr_next, PageRankType *pr_curr, std::mutex* mutex_, CustomBarrier* barrier_){
  ...
  for (uintV v = iterStart; v < iterEnd; v++)
  {
      ...
      for (uintE i = 0; i < in_degree; i++)
      {
        ...
        if (u_out_degree > 0)
        {
          pr_next[v] += (pr_curr[u] / u_out_degree);
        }
...
void Create_Threads(Graph &g, int max_iters, uint n_threads, CustomBarrier &barrier_) {
  ...
  std::mutex* mutex_ = new std::mutex[n];
  ...
  for (int iter = 0; iter < max_iters; iter++)
  {
   for(uint i = 0; i < n_threads - 1; i++)
   {
     ...
     thread_array[i] = std::thread(page_rank_pull_parallel, &g, start, end, thread_id, n, &thread_time[thread_id], std::ref(pr_next), std::ref(pr_curr), std::ref(mutex_), &barrier_);
   }

   start = n/n_threads*(last_thread);
   end = n;
   thread_array[last_thread] = std::thread(page_rank_pull_parallel, &g, start, end, last_thread, n, &thread_time[last_thread], std::ref(pr_next), std::ref(pr_curr), std::ref(mutex_), &barrier_);
  ...
  // 1, 0.12
  for(uint i = 0; i < n_threads; i++){
   std::cout <<i<< ", " << std::setprecision(TIME_PRECISION) << thread_time[i]<<"\n";
  }
