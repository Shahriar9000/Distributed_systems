...
void page_rank_push_parallel(Graph *g, uint iterStart, uint iterEnd, uint thread_id, uint n, double *thread_time, PageRankType *pr_next, PageRankType *pr_curr, std::mutex* mutex_, CustomBarrier* barrier_1){
...
void Create_Threads(Graph &g, int max_iters, uint n_threads, CustomBarrier &barrier_1 , CustomBarrier &barrier_2) {
  uintV n = g.n_;
  ...
  for (int iter = 0; iter < max_iters; iter++)
  {
    ...
    for(uint i = 0; i < n_threads; i++){
     ...
     thread_array[i] = std::thread(page_rank_push_parallel, &g, start, end, thread_id, n, &thread_time[thread_id], std::ref(pr_next), std::ref(pr_curr), std::ref(mutex_), &barrier_1);
   }
...
int main(int argc, char *argv[]) {
  ...
  CustomBarrier barrier_1(n_threads);
  CustomBarrier barrier_2(n_threads);


  Create_Threads(g, max_iterations, n_threads, barrier_1, barrier_2);
